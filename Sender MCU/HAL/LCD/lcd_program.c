...
void LCD_8bit_vShiftDisplay(const LCD_8bit_cfg *PTR_LCD, enu_Shift_direction Copy_enuShiftDirection, u8 Copy_u8ShiftCount){
	switch(Copy_enuShiftDirection){
	case LCD_ShiftLeft:
		for(int i=0;i<Copy_u8ShiftCount;i++){
			LCD_8bit_vSendCmd(PTR_LCD, CMD_SHIFT_LEFT);
		}
		break;
	case LCD_ShiftRight:
		for(int i=0;i<Copy_u8ShiftCount;i++){
			LCD_8bit_vSendCmd(PTR_LCD, CMD_SHIFT_RIGHT);
		}
		break;
...
void LCD_4bit_vShiftDisplay(const LCD_4bit_cfg *PTR_LCD, enu_Shift_direction Copy_enuShiftDirection, u8 Copy_u8ShiftCount){
	switch(Copy_enuShiftDirection){
	case LCD_ShiftLeft:
		for(int i=0;i<Copy_u8ShiftCount;i++){
			LCD_4bit_vSendCmd(PTR_LCD, CMD_SHIFT_LEFT);
		}
		break;
	case LCD_ShiftRight:
		for(int i=0;i<Copy_u8ShiftCount;i++){
			LCD_4bit_vSendCmd(PTR_LCD, CMD_SHIFT_RIGHT);
		}
		break;
	default: /* Do Nothing */ break;
	}
